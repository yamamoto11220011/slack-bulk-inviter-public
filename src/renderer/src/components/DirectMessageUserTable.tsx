...
import type { UserFilter, UserSort } from '../stores/app-store'
...
export function DirectMessageUserTable({
  users,
  categories,
  ...
  const rowProps = useMemo(
    () => ({
      users: filteredUsers,
      selectedUserIds,
      toggleUser: onToggleUser,
      categoryLabelMap
    }),
    [filteredUsers, selectedUserIds, onToggleUser, categoryLabelMap]
  )

  const handleSelectAll = () => {
    if (allSelected) {
      onDeselectUsers(visibleIds)
    } else {
      onSelectUsers(visibleIds)
    }
  }
      ...
      >
        ...
        <div className="w-32 px-4 py-2 shrink-0 overflow-hidden">
          <div className="flex flex-col items-start gap-1">
            ...
            >
              {hasStudentId(user.name) ? 'Student' : 'Check'}
            </span>
          </div>
        </div>
        <div className="w-40 px-4 py-2 shrink-0 overflow-hidden">
          {categoryLabel ? (
            ...
            <span className="inline-block truncate max-w-full rounded bg-amber-100 px-2 py-0.5 text-[11px] font-medium text-amber-700">
              要確認
            </span>
          )}
        </div>
